/*
 * This code was written to support the book, "ARM Assembly for Embedded
 *  Applications", by Daniel W. Lewis. Permission is granted to freely share
 *  this software provided that this notice is not removed. This software is
 *  intended to be used with a run-time library adapted by the author from the
 *  STM Cube Library for the 32F429IDISCOVERY board and available for download
 *  from http://www.engr.scu.edu/~dlewis/book3.
**/
        .syntax unified
        .cpu cortex-m4
        .text
...
EccentricAnomaly: // S0 = e, S1 = M
        PUSH {LR}
        VPUSH {S16-S18}
        vmov.f32 s16, s0          // S16 = e
        vmov.f32 s17, s1          // S17 = M
        vmov.f32 s0,  s1          // S0 = M
        BL cosDeg             // S0 = cosDeg(M)
        vmov.f32 s18, 1.0         // S18 = 1.0
        vmla.f32 s18, s0, s16 // S18 = 1.0 + e * cosDeg(M)
        vmov.f32 s0, s17          // S0 = M
